...
async function extractAllSubsNoDecode(pipeIndex, outputFile, label) {
  ...
  await new Promise((resolve, reject) => {
    // Use -vn -an to skip video/audio decode - much faster for subtitle-only extraction
...
async function main() {
  console.log('Starting subtitle extraction (no decode mode)...');
  await extractAllSubsNoDecode(2, '/tmp/scene04_ep04_full_kr.srt', 'Korean');
  console.log('Korean done. Starting Chinese...');
  await extractAllSubsNoDecode(3, '/tmp/scene04_ep04_full_cn.srt', 'Chinese Simplified');
