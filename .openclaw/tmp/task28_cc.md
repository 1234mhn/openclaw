你是韩语课程编辑。请制作**【第28课】《一眼看穿》** 的复利笔记本 PDF，严格按定稿模板 + 最新 SOP。第28课只有 **5 句台词**（小课），务必紧凑、无孤行页、页面填满。
...
- 素材源（台词/单词/句型/音变，从这里提取全部内容）：`/root/.openclaw/workspace/obsidian_nanan_compound_library/语言库/韩语库/这爱情怎么翻译 课程页面/lesson28.html`
...
## 本课内容（第28课共5句，全部逐字照 lesson28.html）
...
音频：https://nannan2026.cn/korean/audio/L28_S01.mp3 … L28_S05.mp3 + L28_full.mp3

本课句型（重点7个）：-고(又…又…) / -다고 하다(间接引用"说") / -라고 하다(命令引用"让") / -처럼(像…一样) / -ㄹ 겁니다(将来推测) / -겠죠(推测+确认"应该…吧") / 句尾-고省略(口语未完句)

本课单词（lesson28.html 的 vocab 全部，约19个）：차무희 상당히 즉흥적 변덕스럽다 방금 내놓다 우기다 바꾸다 그런 불안하다 도라미 나서다 지난번 터지다 분명히 그렇다 그럼 사정 알다

## 页面结构（按SOP"小课页面分配"，5句课目标约8-11页）
1. 封面：E3-28 · 韩语台词复利笔记；标题"一眼看穿"；韩文给一句提炼（可"상대를 꿰뚫는 눈"或照台词主题）；副题贴合"看穿/直接点破/直言不讳"；SVG插画**主题=洞察/看穿/犀利目光**（如深邃眼瞳+锐利线条，不要用模板极光图）
2. 台词汇总：5句 + 整段音频 L28_full；若只占上半页→按SOP补「重点句型速览」表格+核心词汇填满
3. 每句一句精讲页（句子框架sf-card / 单词 / 短语 / 句型 / 音变 + 应用：换词/仿写/Q&A）
4. 重点句型表+重点单词表（同页；7个句型；6个重点单词带联想记忆）
5. 场景迁移：2个A/B对话，各4-6轮，三语分行（主题"直言相告/直接点破/推心置腹"）
6. 场景日记(5句)+口语挑战 同页

## 铁律（SOP重点，本次务必遵守）
- **5句小课：每句精讲压成1页（compact-page），绝不产生孤行页**；若某句最后Q&A溢到新页只剩几行+页脚→必须压缩重排回一页
- 页面尽量填满，无大片空白、无孤行页；续页顶部空一段不顶格
- 词尾四种：用到的只✓无注释、没用到`<s>标签</s>`删除线贯穿；不要sf-detail详解行
- 单词/短语/句型严格分开；句子框架讲解大白话
- 台词①直接紧贴台词（.num窄、.kr不缩进）
- CSS严格照抄模板；header-left：「第28课 · 一眼看穿」
- 渲染后用 pdftoppm 转图逐页核对（SOP第5章自检）：每页内容行≥95%、无孤行、页数合理、无第29/30课残留

## 完成标准
1. 生成 第28课_复利笔记_final.html（内容准确、无残留其他课内容）
2. 渲染 第28课_复利笔记.pdf，目标约8-11页
3. 自检通过（无孤行页、无大空白、词尾规则对）
4. 报告：PDF路径、页数、文件大小
